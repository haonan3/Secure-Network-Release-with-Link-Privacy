...
def read_graph():
	...
	'''
	if args.weighted:
		G = nx.read_edgelist('{}/{}'.format(root_path, args.input), nodetype=int, data=(('weight',float),), create_using=nx.DiGraph())
	else:
		G = nx.read_edgelist('{}/{}'.format(root_path, args.input), nodetype=int, create_using=nx.DiGraph())
		for edge in G.edges():
...
def learn_embeddings(args, walks):
	...
	model.wv.save_word2vec_format('{}/{}'.format(root_path, args.output))
	return model.wv
